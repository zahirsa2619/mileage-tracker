...
$stmt = $pdo->prepare("SELECT e.*, dr.log_date 
    FROM expenses e 
    JOIN daily_records dr ON e.record_id = dr.id 
    WHERE dr.user_id = ? AND dr.log_date BETWEEN ? AND ? 
    ORDER BY dr.log_date ASC");
$stmt->execute([$user_id, $from, $to]);
$exp_list = $stmt->fetchAll();
...
// UC Tiered Mileage Calculation: first 833 miles @ 45p, thereafter 25p
$total_miles = $t['m'];
if ($total_miles <= 833) {
    $mile_allow = $total_miles * 0.45;
} else {
    $mile_allow = (833 * 0.45) + (($total_miles - 833) * 0.25);
}
...
<div class="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-md border print:shadow-none print:border-none">
    ...
    <div class="grid grid-cols-4 gap-2 mb-6 text-center">
        ...
        <div class="p-2 border rounded bg-slate-50 box">
            <div class="text-[9px] text-gray-400 uppercase">Mileage Allowance</div>
            <div class="text-[8px] text-gray-500">
                <?php if ($total_miles <= 833): ?>
                    <?= $total_miles ?> mi @ 45p
                <?php else: ?>
                    833 @ 45p + <?= ($total_miles - 833) ?> @ 25p
                <?php endif; ?>
            </div>
            <div class="text-xs font-bold">£<?= number_format($mile_allow, 2) ?></div>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div>
            <h2 class="text-[10px] font-black uppercase text-indigo-400 mb-2">Employer Breakdown</h2>
            <div class="grid grid-cols-1 gap-2">
                ...
                <div class="p-2 border rounded text-[10px] bg-white box">
                    ...
                    <div class="flex justify-between text-red-500"><span>Pension:</span><b>-£<?= number_format($e['total_pension'], 2) ?></b></div>
                    <div class="flex justify-between text-red-500"><span>Fees:</span><b>-£<?= number_format($e['total_fees'], 2) ?></b></div>
                </div>
                <?php endforeach; ?>
            </div>
        </div>
        <div>
            <h2 class="text-[10px] font-black uppercase text-red-400 mb-2">Expense Breakdown</h2>
            <div class="grid grid-cols-1 gap-2">
                <?php foreach($exp_list as $ex): ?>
                <div class="p-2 border rounded text-[10px] bg-white box">
                    <div class="flex justify-between border-b mb-1 pb-1 font-bold text-red-700 uppercase">
                        <span><?= date('d/m', strtotime($ex['log_date'])) ?></span>
                        <span>£<?= number_format($ex['amount'], 2) ?></span>
                    </div>
                    <div class="truncate text-gray-500 italic"><?= htmlspecialchars($ex['description']) ?></div>
                </div>
                <?php endforeach; ?>
                <?php if(empty($exp_list)): ?>
                    <div class="p-2 text-center text-gray-400 text-[10px] italic">No expenses logged.</div>
                <?php endif; ?>
            </div>
        </div>
    </div>
